...
const DIGIT_COUNT = 5;
const TEXT_SUFFIX = " BODIES IN MOTION";
const TOTAL_CHARS = DIGIT_COUNT + TEXT_SUFFIX.length; // 22
const TICK_MS = 75;
const REFRESH_INTERVAL_MS = 10000;

// Uppercase + digits only — matches the all-caps text aesthetic
...
export function MovesCounter({ count, className = "" }: MovesCounterProps) {
  ...
  // Full target: "00032 BODIES IN MOTION"
  const fullTarget =
    formatCount(revealedLength >= TOTAL_CHARS ? displayCount : animationTargetRef.current) +
    TEXT_SUFFIX;

  // Build display string: revealed chars are real, unrevealed are scrambled.
  // Spaces are always shown as spaces so word shapes stay readable during animation.
