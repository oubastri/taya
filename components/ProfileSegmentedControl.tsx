...
function PersonIcon({ color }: { color: string }) {
  return (
    <svg width={ICON_PX} height={ICON_PX} viewBox="0 0 24 24" fill={color} aria-hidden>
  ...
  return (
    ...
    >
      {([
        ...
        const fg = isActive ? "var(--background)" : "var(--foreground)";
        const bg = isActive ? "var(--foreground)" : "var(--surface)";
        return (
          <button
            key={key}
            type="button"
